...
export default function ProductDetailClient({ product }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-[6fr_5fr] gap-10 lg:gap-16 items-start w-full">
      ...
      <div className="flex flex-col py-3 gap-5">
        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          {product.tags && product.tags.some(t => t.toUpperCase() === 'EXHIBITION CATEGORIES') && (
            <span className="px-4 py-1.5 bg-brand-green/10 text-brand-green text-[10px] font-bold tracking-[0.2em] uppercase rounded-full border border-brand-green/20">Exhibition Signature</span>
          )}
        </div>

        {/* Product Name */}
        <h1 className="DM Sans text-2xl md:text-3xl lg:text-[48px] text-brand-green font-medium leading-snug">
          {product.name}
        </h1>

        {/* Price */}
        <div className="flex items-baseline gap-5 pt-6 border-t border-gray-100">
          <span className="font-sans text-[38px] font-bold text-gold tracking-tight">₹{product.price.toLocaleString()}</span>
          {product.originalPrice && product.originalPrice > product.price && (
            <span className="text-gray-400 line-through text-[25px] font-light">₹{product.originalPrice.toLocaleString()}</span>
          )}
        </div>

        {/* Quantity */}
        <div className="pt-2">
          <p className="text-[25px] DMsansC3 text-[#020202] mb-8">Quantity</p>
        ...
        <div className="flex flex-col gap-7 pt-10 ">
          <button
            ...
            className="w-[60%] mx-auto px-4 py-4 rounded-full bg-white/45 text-[#2E4B36] DM sans C3 text-[21px] tracking-[0.1em] hover:bg-brand-green hover:text-white transition-all duration-300 active:scale-95"
          >
            Add to Cart
          </button>
          <button
            onClick={handleBuyNow}
            className="w-[60%] mx-auto px-4 py-4 rounded-full  bg-white/45 text-[#2E4B36] DM sans C3 text-[21px] tracking-[0.1em] hover:bg-brand-green hover:text-white transition-all duration-300 active:scale-95 flex items-center justify-center gap-2"
          >
