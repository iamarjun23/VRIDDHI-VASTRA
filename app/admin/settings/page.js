...
function SettingsPageContent() {
  ...
  const LocalImageUploader = ({ label, desc, image, onUpload, aspect, isUploadingKey, compact = false }) => (
    <div className={`bg-white rounded-[32px] md:rounded-[56px] border border-gray-100/80 shadow-sm hover:shadow-2xl transition-all duration-1000 ${compact ? 'p-6 md:p-12' : 'p-6 md:p-14'} group`}>
      <div className={compact ? 'mb-8 md:mb-12' : 'mb-8 md:mb-14'}>
        <h3 className={`font-bold font-display text-gray-900 ${compact ? 'text-2xl md:text-3xl' : 'text-2xl md:text-4xl'}`}>{label}</h3>
    ...
    <div className="min-h-screen bg-[#F9F8F6]">
      ...
      <header className="sticky top-0 z-40 bg-white/70 backdrop-blur-xl border-b border-gray-100/50 px-6 md:px-10 py-4 md:py-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold font-display text-gray-900 tracking-tight">Curation Studio</h1>
      ...
      <div className="px-4 md:px-10 py-8 md:py-12 space-y-8 md:space-y-12 pb-24 md:pb-48">
        {/* Horizontal Navigation */}
        <nav className="flex items-center gap-2 md:gap-4 p-2 md:p-4 bg-white/80 rounded-[32px] md:rounded-[48px] border border-gray-100 shadow-sm sticky top-[80px] md:top-[92px] z-30 backdrop-blur-2xl overflow-x-auto no-scrollbar">
          {tabs.map(tab => (
            <button
              key={tab.id}
              onClick={() => {
              ...
              className={`flex-1 min-w-[60px] md:min-w-0 flex items-center justify-center gap-3 md:gap-5 px-4 md:px-10 py-3 md:py-5 rounded-[24px] md:rounded-[36px] text-[10px] md:text-[12px] font-bold tracking-[0.2em] md:tracking-[0.3em] uppercase transition-all duration-700 ${activeTab === tab.id ? 'bg-black text-white shadow-2xl shadow-black/10 scale-[1.02]' : 'text-gray-400 hover:text-gray-700 hover:bg-[#F9F8F6]'}`}
            >
        ...
        <main className="animate-in slide-in-from-bottom-8 duration-1000">
          ...
          {activeTab === 'hero' && (
            <div className="space-y-10 md:space-y-16">
              <div className="space-y-4 px-2">
                <h2 className="text-3xl md:text-5xl font-bold font-display text-gray-900 tracking-tight">Identity & Atmosphere</h2>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <div className="bg-white rounded-[32px] md:rounded-[56px] p-6 md:p-14 border border-gray-100 shadow-sm space-y-8 md:space-y-10 group">
                   <div className="flex items-center justify-between">
                      <h3 className="text-2xl md:text-3xl font-bold font-display text-gray-900">Communication</h3>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <div className="bg-white rounded-[32px] md:rounded-[56px] p-6 md:p-14 border border-gray-100 shadow-sm space-y-6 md:space-y-10 flex flex-col">
                  <div>
                    <h3 className="text-2xl md:text-3xl font-bold font-display text-gray-900">Promotional Narrative</h3>
            ...
            <div className="space-y-16">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                {formData.featuredBlocks.map((block, index) => (
                  <div key={`feat-${index}`} className="bg-white rounded-[32px] md:rounded-[56px] p-6 md:p-12 border border-gray-100 shadow-sm hover:shadow-2xl transition-all duration-1000 group">
            ...
            <div className="space-y-16">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                {formData.lookbookBlocks.map((block, index) => (
                  <div key={`look-${index}`} className="bg-white rounded-[32px] md:rounded-[56px] p-6 md:p-12 border border-gray-100 shadow-sm hover:shadow-2xl transition-all duration-1000 group">
                    <div className="flex items-center justify-between mb-8 md:mb-12">
          ...
          {activeTab === 'filters' && (
            <div className="space-y-16">
              ...
              <div className="bg-white rounded-[32px] md:rounded-[64px] p-6 md:p-16 lg:p-20 border border-gray-100 shadow-sm space-y-8 md:space-y-16 relative overflow-hidden group">
                <div className="absolute top-0 right-0 w-64 h-64 md:w-[500px] md:h-[500px] bg-brand-gold/5 rounded-full -mr-20 -mt-20 md:-mr-40 md:-mt-40 group-hover:scale-110 transition-transform duration-[2s]"></div>
                
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 md:gap-10 relative z-10">
                  <div>
                    <h3 className="text-2xl md:text-4xl font-bold font-display text-gray-900 tracking-tight">Active Navigation Tags</h3>
                    <p className="text-sm md:text-lg text-gray-400 mt-2 md:mt-4 font-medium leading-relaxed max-w-xl">The definitive list of categories for sidebar navigation and archive discovery.</p>
                  </div>
                  <button 
                    onClick={() => setFormData(p => ({ ...p, collectionsCategories: [...p.collectionsCategories, ""] }))}
                    className="px-6 md:px-12 py-3 md:py-5 bg-black text-white rounded-[16px] md:rounded-[24px] text-[10px] md:text-[12px] font-bold tracking-[0.3em] md:tracking-[0.4em] uppercase hover:bg-brand-green transition-all shadow-2xl shadow-black/20 active:scale-95 whitespace-nowrap w-full md:w-auto"
                  >
                    + Add New Category
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8 relative z-10">
                  {formData.collectionsCategories.map((cat, index) => (
                    <div key={`col-cat-bottom-${index}`} className="flex items-center gap-3 md:gap-5 animate-in fade-in slide-in-from-left duration-700">
                      <input 
                        type="text" 
                        value={cat} 
                        onChange={(e) => {
                        ...
                        className="flex-1 min-w-0 bg-[#F9F8F6]/80 border-gray-100 border-2 rounded-[16px] md:rounded-[24px] px-4 md:px-8 py-3 md:py-5 font-bold tracking-[0.2em] md:tracking-[0.3em] uppercase text-xs md:text-sm focus:border-brand-green focus:bg-white transition-all outline-none shadow-sm" 
                      />
                      <button 
                        onClick={() => {
                        ...
                        className="p-3 md:p-5 flex-shrink-0 text-red-300 hover:text-red-500 hover:bg-red-50 rounded-[12px] md:rounded-[20px] transition-all shadow-sm group/del"
                      >
                        <svg className="w-5 h-5 md:w-6 md:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                      </button>
                    </div>
                  ))}
                </div>
          ...
          {activeTab === 'contact' && (
            <div className="space-y-16">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <div className="bg-white rounded-[32px] md:rounded-[56px] p-6 md:p-14 border border-gray-100 shadow-sm flex flex-col justify-center relative group overflow-hidden">
                   <div className="absolute top-0 right-0 w-48 h-48 md:w-64 md:h-64 bg-brand-gold/5 rounded-full -mr-16 -mt-16 md:-mr-24 md:-mt-24 group-hover:scale-150 transition-transform duration-1000"></div>
                   <div className="relative z-10">
                      <h3 className="text-2xl md:text-3xl font-bold font-display text-gray-900">Environmental Essence</h3>
                      <p className="text-sm md:text-lg text-gray-400 mt-4 md:mt-6 leading-relaxed font-medium">The contact portal photography serves as the foundational depth for the glassmorphic interface. Select imagery that conveys the raw heritage and cinematic texture of the brand.</p>
                      
                      <div className="mt-8 md:mt-14 p-6 md:p-10 bg-[#F9F8F6]/80 rounded-[24px] md:rounded-[40px] border border-gray-100 shadow-inner">
                         <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <span className="text-xs md:text-md text-gray-500 font-bold tracking-[0.2em] uppercase">Render Engine:</span>
                            <span className="text-xs md:text-sm font-bold text-gray-900 tracking-[0.2em] md:tracking-[0.4em] uppercase bg-white px-4 md:px-8 py-2 md:py-3.5 rounded-[16px] md:rounded-full border border-gray-100 shadow-sm md:w-auto w-max">GLASS-ARCH V2</span>
                         </div>
                      </div>
                   </div>
                </div>
