...
function SettingsPageContent() {
  ...
  const LocalImageUploader = ({ label, desc, image, onUpload, aspect, isUploadingKey, compact = false }) => (
    <div className={`bg-white rounded-[56px] border border-gray-100/80 shadow-sm hover:shadow-2xl transition-all duration-1000 ${compact ? 'p-12' : 'p-14'} group`}>
      <div className={compact ? 'mb-12' : 'mb-14'}>
        <h3 className={`font-bold font-display text-gray-900 ${compact ? 'text-3xl' : 'text-4xl'}`}>{label}</h3>
        {desc && <p className="text-lg text-gray-400 mt-4 font-medium leading-relaxed">{desc}</p>}
      </div>
      
      <div className={compact ? 'space-y-10' : 'space-y-16'}>
        <div className={`relative ${aspect} w-full rounded-[48px] overflow-hidden bg-[#F9F8F6]/50 border border-gray-100 flex items-center justify-center group-hover:border-brand-gold/30 shadow-inner transition-all duration-1000 cursor-pointer`}>
          <input 
            type="file" 
            accept="image/*" 
            onChange={onUpload} 
          ...
          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-700 flex items-center justify-center pointer-events-none z-10">
             <span className="text-[12px] font-bold tracking-[0.5em] text-white uppercase bg-black/30 backdrop-blur-xl px-12 py-5 rounded-full border border-white/20 shadow-2xl scale-95 group-hover:scale-100 transition-transform duration-700">Sync Archive</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="min-h-screen bg-[#F9F8F6]">
      ...
      <header className="sticky top-0 z-40 bg-white/70 backdrop-blur-xl border-b border-gray-100/50 px-10 py-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold font-display text-gray-900 tracking-tight">Curation Studio</h1>
            <p className="text-[11px] text-brand-green/70 tracking-[0.4em] uppercase font-bold mt-1.5 ml-0.5">Refining Vriddhi Vastra Aesthetics</p>
          </div>
          <div className="flex items-center gap-8">
            {message.text && (
              <span className={`px-8 py-3 rounded-full text-[11px] font-bold tracking-[0.3em] uppercase animate-in fade-in slide-in-from-right duration-700 shadow-sm border ${message.type === 'error' ? 'bg-red-50 text-red-600 border-red-100' : 'bg-green-50 text-green-600 border-green-100'}`}>
                {message.text}
              </span>
            )}
            <button
              ...
              className="bg-black hover:bg-brand-green text-white px-12 py-4 rounded-full text-[11px] font-bold tracking-[0.4em] uppercase transition-all shadow-2xl shadow-black/20 active:scale-95 disabled:opacity-50"
            >
      ...
      <div className="px-10 py-12 space-y-12 pb-48">
        {/* Horizontal Navigation */}
        <nav className="flex items-center gap-4 p-4 bg-white/80 rounded-[48px] border border-gray-100 shadow-sm sticky top-[92px] z-30 backdrop-blur-2xl">
          {tabs.map(tab => (
            <button
              key={tab.id}
              onClick={() => {
              ...
              className={`flex-1 flex items-center justify-center gap-5 px-10 py-5 rounded-[36px] text-[12px] font-bold tracking-[0.3em] uppercase transition-all duration-700 ${activeTab === tab.id ? 'bg-black text-white shadow-2xl shadow-black/10 scale-[1.02]' : 'text-gray-400 hover:text-gray-700 hover:bg-[#F9F8F6]'}`}
            >
        ...
        <main className="animate-in slide-in-from-bottom-8 duration-1000">
          ...
          {activeTab === 'hero' && (
            <div className="space-y-16">
              <div className="space-y-4">
                <h2 className="text-5xl font-bold font-display text-gray-900 tracking-tight">Identity & Atmosphere</h2>
                <p className="text-gray-500 text-lg font-medium max-w-2xl leading-relaxed">Coordinate the primary visual assets and brand identity of your digital vault.</p>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <div className="bg-white rounded-[56px] p-14 border border-gray-100 shadow-sm space-y-10 group">
                   <div className="flex items-center justify-between">
                      <h3 className="text-3xl font-bold font-display text-gray-900">Communication</h3>
                   ...
                   <div className="space-y-6">
                      <label className="text-[12px] font-bold tracking-[0.4em] text-gray-400 uppercase block pl-2">WhatsApp Contact Number</label>
                      <input 
                        type="text" 
                        placeholder="e.g. 919000000000"
                        value={formData.whatsappNumber} 
                        onChange={e => setFormData(p => ({ ...p, whatsappNumber: e.target.value }))}
                        className="w-full bg-[#F9F8F6]/50 border-gray-100 border-2 rounded-[24px] px-8 py-5 text-2xl font-bold font-sans text-gray-900 focus:border-brand-green focus:bg-white transition-all outline-none shadow-sm" 
                      />
                      <p className="text-sm text-gray-400 font-medium pl-2 italic">Format: Country code followed by number (e.g., 91 for India).</p>
                   </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <div className="bg-white rounded-[56px] p-14 border border-gray-100 shadow-sm space-y-10 flex flex-col">
                  <div>
                    <h3 className="text-3xl font-bold font-display text-gray-900">Promotional Narrative</h3>
                    <p className="text-lg text-gray-400 mt-3 font-medium leading-relaxed">Widescreen curated promotion for high-impact discovery.</p>
                  </div>
                  
                  <div className="relative aspect-[21/9] w-full rounded-[40px] overflow-hidden bg-[#F9F8F6]/50 border border-gray-100 group shadow-inner">
                  ...
                  <div className="space-y-8 flex-1 flex flex-col justify-end">
                    <input 
                      ...
                      className="w-full bg-[#F9F8F6]/50 border-gray-100 border-2 rounded-[24px] px-8 py-5 text-2xl font-bold font-display text-gray-900 focus:border-brand-green focus:bg-white transition-all outline-none shadow-sm" 
                    />
                    <textarea 
                      rows={3}
                      ...
                      className="w-full bg-[#F9F8F6]/50 border-gray-100 border-2 rounded-[32px] px-8 py-6 text-lg text-gray-600 font-medium focus:border-brand-green focus:bg-white transition-all outline-none resize-none shadow-sm leading-relaxed" 
                    />
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-16">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                {formData.featuredBlocks.map((block, index) => (
                  <div key={`feat-${index}`} className="bg-white rounded-[56px] p-12 border border-gray-100 shadow-sm hover:shadow-2xl transition-all duration-1000 group">
                    <div className="flex items-center justify-between mb-12">
                      <span className="text-[11px] font-bold tracking-[0.5em] uppercase text-brand-gold bg-brand-gold/5 px-8 py-3.5 rounded-full border border-brand-gold/10">ARCHETYPE 0{index + 1}</span>
                    </div>
                    <div className="space-y-12">
                      <div className="relative group/circle w-full aspect-square max-w-[320px] mx-auto">
                        <div className="w-full h-full rounded-full overflow-hidden border-2 border-dashed border-gray-100 group-hover/circle:border-brand-green transition-all duration-1000 relative flex items-center justify-center bg-[#F9F8F6]/50 shadow-inner">
                          {uploadingFields[`featuredBlocks-${index}`] && (
                            <div className="absolute inset-0 z-20 bg-white/70 flex items-center justify-center rounded-full backdrop-blur-xl">
                              <div className="animate-spin rounded-full h-14 w-14 border-b-2 border-brand-green"></div>
                            </div>
                          )}
                          ...
                          {block.image ? (
                            <img src={block.image} className="w-full h-full object-contain p-10 transform group-hover/circle:scale-110 transition-transform duration-[1.5s]" />
                          ) : (
                            <svg className="w-16 h-16 text-gray-100/50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="0.5" d="M12 4v16m8-8H4" /></svg>
                          )}
                        </div>
                      </div>
                      
                      <div className="space-y-6 pt-4">
                        <label className="text-[12px] font-bold tracking-[0.4em] text-gray-400 uppercase block pl-2">Collection Identity</label>
                        <input 
                          type="text" 
                          placeholder="e.g. KANCHIPURAM LUXURY" 
                          value={block.title} 
                          onChange={(e) => handleBlockChange("featuredBlocks", index, "title", e.target.value)} 
                          className="w-full bg-[#F9F8F6]/50 border-gray-100 border-2 rounded-[24px] px-8 py-5 font-bold font-display text-3xl tracking-tight focus:border-brand-green focus:bg-white transition-all outline-none shadow-sm" 
                        />
                      </div>
                    </div>
                  </div>
            ...
            <div className="space-y-16">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                {formData.lookbookBlocks.map((block, index) => (
                  <div key={`look-${index}`} className="bg-white rounded-[56px] p-12 border border-gray-100 shadow-sm hover:shadow-2xl transition-all duration-1000 group">
                    <div className="flex items-center justify-between mb-12">
                      <span className="text-[11px] font-bold tracking-[0.5em] uppercase text-brand-green bg-brand-green/5 px-8 py-3.5 rounded-full border border-brand-green/10">TILE ARCHIVE 0{index + 1}</span>
                    </div>
                    <div className="space-y-12">
                      <div className="relative group/tile w-full aspect-[16/10] rounded-[48px] overflow-hidden bg-[#F9F8F6]/50 border border-gray-100 flex items-center justify-center shadow-inner transition-all duration-1000 cursor-pointer">
                        ...
                        {block.image ? (
                          ...
                          <div className="flex flex-col items-center gap-8">
                            <svg className="w-20 h-20 text-gray-100/50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="0.5" d="M12 4v16m8-8H4" /></svg>
                            <span className="text-[12px] font-bold text-gray-200 uppercase tracking-[0.6em]">Synchronize Motion</span>
                          </div>
                        )}
                      </div>
                      <div className="space-y-6 pt-4">
                        <label className="text-[12px] font-bold tracking-[0.4em] text-gray-400 uppercase block pl-2">Exhibition Title</label>
                        <input 
                          type="text" 
                          placeholder="e.g. BRIDAL COLLECTION" 
                          value={block.title} 
                          onChange={(e) => handleBlockChange("lookbookBlocks", index, "title", e.target.value)} 
                          className="w-full bg-[#F9F8F6]/50 border-gray-100 border-2 rounded-[24px] px-8 py-5 font-bold font-display text-4xl tracking-tight focus:border-brand-green focus:bg-white transition-all outline-none shadow-sm" 
                        />
                      </div>
                    </div>
                  </div>
          ...
          {activeTab === 'filters' && (
            <div className="space-y-16">
              ...
              <div className="bg-white rounded-[64px] p-16 lg:p-20 border border-gray-100 shadow-sm space-y-16 relative overflow-hidden group">
                <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-brand-gold/5 rounded-full -mr-40 -mt-40 group-hover:scale-110 transition-transform duration-[2s]"></div>
                
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-10 relative z-10">
                  <div>
                    <h3 className="text-4xl font-bold font-display text-gray-900 tracking-tight">Active Navigation Tags</h3>
                    <p className="text-lg text-gray-400 mt-4 font-medium leading-relaxed max-w-xl">The definitive list of categories for sidebar navigation and archive discovery.</p>
                  </div>
                  <button 
                    onClick={() => setFormData(p => ({ ...p, collectionsCategories: [...p.collectionsCategories, ""] }))}
                    className="px-12 py-5 bg-black text-white rounded-[24px] text-[12px] font-bold tracking-[0.4em] uppercase hover:bg-brand-green transition-all shadow-2xl shadow-black/20 active:scale-95 whitespace-nowrap"
                  >
                    + Add New Category
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 relative z-10">
                  {formData.collectionsCategories.map((cat, index) => (
                    <div key={`col-cat-bottom-${index}`} className="flex items-center gap-5 animate-in fade-in slide-in-from-left duration-700">
                      <input 
                        type="text" 
                        value={cat} 
                        onChange={(e) => {
                        ...
                        className="flex-1 bg-[#F9F8F6]/80 border-gray-100 border-2 rounded-[24px] px-8 py-5 font-bold tracking-[0.3em] uppercase text-sm focus:border-brand-green focus:bg-white transition-all outline-none shadow-sm" 
                      />
                      <button 
                        onClick={() => {
                        ...
                        className="p-5 text-red-300 hover:text-red-500 hover:bg-red-50 rounded-[20px] transition-all shadow-sm group/del"
                      >
                        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                      </button>
                    </div>
                  ))}
                </div>
          ...
          {activeTab === 'contact' && (
            <div className="space-y-16">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <div className="bg-white rounded-[56px] p-14 border border-gray-100 shadow-sm flex flex-col justify-center relative group overflow-hidden">
                   <div className="absolute top-0 right-0 w-64 h-64 bg-brand-gold/5 rounded-full -mr-24 -mt-24 group-hover:scale-150 transition-transform duration-1000"></div>
                   <div className="relative z-10">
                      <h3 className="text-3xl font-bold font-display text-gray-900">Environmental Essence</h3>
                      <p className="text-lg text-gray-400 mt-6 leading-relaxed font-medium">The contact portal photography serves as the foundational depth for the glassmorphic interface. Select imagery that conveys the raw heritage and cinematic texture of the brand.</p>
                      
                      <div className="mt-14 p-10 bg-[#F9F8F6]/80 rounded-[40px] border border-gray-100 shadow-inner">
                         <div className="flex items-center justify-between">
                            <span className="text-md text-gray-500 font-bold tracking-[0.2em] uppercase">Render Engine:</span>
                            <span className="text-sm font-bold text-gray-900 tracking-[0.4em] uppercase bg-white px-8 py-3.5 rounded-full border border-gray-100 shadow-sm">GLASS-ARCH V2</span>
                         </div>
                      </div>
                   </div>
                </div>
