    ...
    <div className="min-h-screen bg-[#F9F8F6]">
      ...
      <header className="sticky top-0 z-30 bg-white/70 backdrop-blur-xl border-b border-gray-100/50 px-6 md:px-10 py-4 md:py-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
             <h1 className="text-2xl md:text-3xl font-bold font-display text-gray-900 tracking-tight">Command Center</h1>
      ...
      <main className="px-4 md:px-10 py-8 md:py-12 space-y-8 md:space-y-16 pb-24 md:pb-32 animate-in slide-in-from-bottom-6 duration-1000">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-10">
          {[
            ...
            <div key={stat.label} className="bg-white p-6 md:p-10 rounded-[32px] md:rounded-[56px] border border-gray-100/80 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-700 group">
              <p className={`text-[10px] md:text-[12px] font-bold tracking-[0.2em] md:tracking-[0.4em] text-gray-400 uppercase group-hover:text-${stat.color} transition-colors`}>{stat.label}</p>
              <h3 className="text-2xl md:text-5xl font-bold font-display text-gray-900 mt-3 md:mt-6 tracking-tighter">{stat.value}</h3>
              <div className={`w-8 md:w-16 h-[2px] md:h-[3px] bg-gray-100 mt-4 md:mt-8 group-hover:w-full group-hover:bg-${stat.color}/20 transition-all duration-1000`}></div>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 md:gap-12">
          ...
          <div className="xl:col-span-2 bg-white rounded-[40px] md:rounded-[64px] shadow-sm border border-gray-100/80 overflow-hidden flex flex-col group/card hover:shadow-2xl transition-all duration-1000">
            <div className="px-6 md:px-14 py-8 md:py-12 border-b border-gray-50 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-gray-50/20">
              <div>
                <h3 className="text-2xl md:text-3xl font-bold font-display text-gray-900 tracking-tight">Recent Masterpieces</h3>
                  ...
                  recentProducts.map((p) => (
                    <li key={p._id.toString()} className="px-4 md:px-10 py-6 md:py-8 rounded-[24px] md:rounded-[40px] hover:bg-[#F9F8F6] transition-all flex flex-col md:flex-row items-start md:items-center gap-6 md:gap-12 group">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xl md:text-2xl font-bold font-display text-gray-900 md:truncate tracking-tight">{p.name}</p>
          ...
          <div className="bg-white rounded-[40px] md:rounded-[64px] shadow-sm border border-gray-100/80 overflow-hidden flex flex-col group/card hover:shadow-2xl transition-all duration-1000">
              ...
              <div className="space-y-4 md:space-y-6">
                {recentSubmissions.length === 0 ? (
                  ...
                  recentSubmissions.map((s) => (
                    <div key={s._id.toString()} className="p-6 md:p-10 rounded-[24px] md:rounded-[40px] bg-[#F9F8F6]/50 hover:bg-[#F9F8F6] border border-transparent hover:border-gray-100 transition-all group">
                      <div className="flex justify-between items-start mb-3 md:mb-5">
                        <p className="text-lg md:text-xl font-bold font-display text-gray-900 tracking-tight">{s.name}</p>
                        <span className="text-[9px] md:text-[10px] text-gray-400 font-bold uppercase tracking-[0.2em]">{new Date(s.createdAt).toLocaleDateString()}</span>
                      </div>
                      <p className="text-[10px] md:text-[11px] font-bold text-brand-gold uppercase tracking-[0.3em] mb-3 md:mb-5">{s.phoneNumber}</p>
                      <div className="p-4 md:p-6 bg-white rounded-2xl md:rounded-3xl border border-gray-100/50 shadow-sm italic text-gray-500 text-[10px] md:text-xs leading-relaxed group-hover:border-brand-gold/20 transition-all font-medium">
                        "{s.message}"
                      </div>
                    </div>
                  ))
            ...
            <div className="p-6 md:p-10 border-t border-gray-50">
               <Link href="/admin/inquiries" className="flex items-center justify-center gap-3 md:gap-4 w-full py-4 md:py-5 bg-gray-900 text-[10px] md:text-[11px] font-bold tracking-[0.3em] md:tracking-[0.4em] text-white uppercase rounded-[24px] md:rounded-[32px] hover:bg-brand-gold transition-all shadow-2xl shadow-black/10 active:scale-95 group">
                 Open Message Vault <svg fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24" className="w-3.5 h-3.5 md:w-4 md:h-4 group-hover:translate-x-2 transition-transform duration-500"><path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3" /></svg>
               </Link>
            </div>
          </div>
        </div>
