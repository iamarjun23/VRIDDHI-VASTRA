...
function InquiriesAdminContent() {
  ...
  return (
    <div className="flex flex-col animate-in fade-in zoom-in-95 duration-500 pb-12">
      ...
      {/* Luxury Header */}
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 sm:gap-6 border-b border-[#E5E0D8] pb-6 shrink-0 px-1">
        <div>
          <span className="text-[10px] font-bold text-[#D4AF37] uppercase tracking-[0.3em] block mb-2">Concierge Desk</span>
          <h1 className="text-[clamp(1.5rem,3vw,2.5rem)] font-bold font-display text-gray-900 leading-none">Inquiries</h1>
        </div>
      ...
      <div className="flex flex-col lg:flex-row flex-1 overflow-hidden bg-white border-y sm:border border-[#E5E0D8] shadow-xl shadow-black/5 mt-4 sm:mt-6 rounded-none min-h-[500px] lg:min-h-0 -mx-3 sm:mx-0">
        
        {/* Left Pane - List */}
        <div className={`w-full lg:w-[320px] xl:w-[380px] shrink-0 lg:border-r border-[#E5E0D8] bg-[#FAF9F6] flex-col z-10 shadow-[0_4px_24px_rgba(0,0,0,0.02)] lg:shadow-[4px_0_24px_rgba(0,0,0,0.02)] ${isMobileDetailView ? 'hidden lg:flex' : 'flex'}`}>
        ...
        {/* Right Pane - Detail */}
        <div className={`flex-1 bg-white flex-col relative ${!isMobileDetailView ? 'hidden lg:flex' : 'flex'}`}>
          {activeInquiry ? (
            <div key={activeInquiry._id} className="animate-in fade-in slide-in-from-right-4 duration-500 h-full flex flex-col">
              {/* Detail Header */}
              <div className="p-5 sm:p-10 lg:p-16 border-b border-[#E5E0D8] bg-[#FAF9F6] shrink-0 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex items-center gap-4 sm:gap-6">
                  <button onClick={() => setIsMobileDetailView(false)} className="lg:hidden w-10 h-10 flex items-center justify-center shrink-0 border border-[#E5E0D8] bg-white text-gray-500 hover:text-gray-900 shadow-sm mr-1">
                  ...
                  <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full flex items-center justify-center text-[18px] sm:text-[20px] font-bold shrink-0 bg-[#1A1A1A] text-[#D4AF37] shadow-xl shadow-black/5">
                    {activeInquiry.name.charAt(0).toUpperCase()}
                  </div>
                  <div>
                    <h2 className="text-[24px] sm:text-[32px] font-bold font-display text-gray-900 tracking-tight leading-tight">{activeInquiry.name}</h2>
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1">
                      <span className="text-[9px] font-bold uppercase tracking-luxury text-[#D4AF37]">
                        {activeInquiry.source || 'Direct Contact'}
                      </span>
                      <span className="text-[10px] text-gray-400 font-medium tracking-luxury uppercase">
                ...
                <div className="flex items-center gap-3">
                  {activeInquiry.phoneNumber && (
                    <a 
                      ...
                      className="px-6 py-3.5 bg-[#25D366] hover:bg-[#1EBE53] text-white text-[10px] font-bold uppercase tracking-luxury shadow-xl shadow-green-500/10 transition-all duration-300 active:scale-95 flex items-center gap-2"
                    >
                  ...
                  <button 
                    ...
                    className="w-12 h-12 flex items-center justify-center text-red-400 hover:text-red-600 hover:bg-red-50 transition-all duration-300"
                  >
              ...
              {/* Message Content */}
              <div className="flex-1 overflow-y-auto p-5 sm:p-10 lg:p-16 custom-scrollbar">
                
                <div className="max-w-4xl space-y-12">
                   
                   {/* Contact Profile */}
                   <div className="space-y-6">
                      <h3 className="text-[9px] font-bold text-[#D4AF37] uppercase tracking-luxury border-b border-[#E5E0D8] pb-4">Client Contact Profile</h3>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        {activeInquiry.phoneNumber && (
                          <div className="bg-white border border-[#E5E0D8] p-5 flex items-center justify-between group hover:border-[#D4AF37] transition-all duration-300">
                            <div className="flex items-center gap-4">
                              <div className="w-10 h-10 bg-[#FAF9F6] border border-[#E5E0D8] flex items-center justify-center shrink-0">
                                <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                              </div>
                              <div>
                                <span className="text-[9px] font-bold text-gray-400 uppercase tracking-luxury block mb-1">Phone Number</span>
                                <span className="text-[14px] font-mono text-gray-900 font-medium">{activeInquiry.phoneNumber}</span>
                              </div>
                            </div>
                            <button onClick={() => copyToClipboard(activeInquiry.phoneNumber)} className="p-2 text-gray-300 hover:text-[#D4AF37] transition-colors" title="Copy">
                        ...
                        {activeInquiry.email && (
                          <div className="bg-white border border-[#E5E0D8] p-5 flex items-center justify-between group hover:border-[#D4AF37] transition-all duration-300">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                <span className="text-[9px] font-bold text-gray-400 uppercase tracking-luxury block mb-1">Email Address</span>
                                <span className="text-[14px] text-gray-900 font-medium truncate max-w-[150px] sm:max-w-none block">{activeInquiry.email}</span>
                              </div>
                            </div>
                            <button onClick={() => copyToClipboard(activeInquiry.email)} className="p-2 text-gray-300 hover:text-[#D4AF37] transition-colors" title="Copy">
                              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
                            </button>
                          </div>
                        )}
                   ...
                   {activeInquiry.productName && (
                     <div className="space-y-6">
                       <h3 className="text-[9px] font-bold text-[#D4AF37] uppercase tracking-luxury border-b border-[#E5E0D8] pb-4">Collection Piece Context</h3>
                       <div className="bg-white border border-[#E5E0D8] p-6 flex items-center gap-5">
                         ...
                         <div>
                           <p className="text-[18px] font-bold font-display text-gray-900 tracking-tight leading-none mb-2">{activeInquiry.productName}</p>
                           <p className="text-[10px] font-bold tracking-luxury text-gray-400 uppercase">Archive Serial: {activeInquiry.productSerial}</p>
                         </div>
                       </div>
                     </div>
                   )}
                   ...
                   {/* Message */}
                   <div className="space-y-6">
                      <h3 className="text-[9px] font-bold text-[#D4AF37] uppercase tracking-luxury border-b border-[#E5E0D8] pb-4">Message Transcript</h3>
                      <div className="bg-[#FAF9F6] border border-[#E5E0D8] p-6 sm:p-10">
                        <p className="text-[16px] md:text-[18px] text-gray-800 whitespace-pre-wrap font-medium leading-mobile-relaxed sm:leading-relaxed">
