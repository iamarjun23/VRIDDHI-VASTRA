...
export default function ProductCard({ product, bgWhite = false }) {
  ...
  return (
    <div className={`group relative overflow-hidden transition-all duration-700 flex flex-col h-full rounded-[clamp(20px,4vw,40px)] ${bgWhite ? 'bg-white shadow-sm hover:shadow-md' : ''}`}>
      ...
      <div className="relative overflow-hidden aspect-[4/5] rounded-t-[clamp(20px,4vw,40px)] bg-black/[0.03]">
        ...
        <div className="absolute inset-x-4 bottom-4 translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 z-10 hidden sm:flex flex-col gap-2">
          <button
            ...
            className="w-full py-3 bg-white/90 backdrop-blur-md text-black rounded-full font-sans text-[clamp(9px,1.2vw,11px)] font-bold tracking-[0.1em] uppercase hover:bg-black hover:text-white transition-all shadow-lg active:scale-95"
          >
            Add to Cart
          </button>
          <button
            onClick={handleWhatsAppBuyNow}
            className="w-full py-3 bg-brand-green text-white rounded-full font-sans text-[clamp(9px,1.2vw,11px)] font-bold tracking-[0.1em] uppercase hover:bg-opacity-90 transition-all shadow-lg active:scale-95 flex items-center justify-center gap-2"
          >
      ...
      <div className={`p-5 flex flex-col items-center flex-1 ${bgWhite ? 'px-6 pb-6' : ''}`}>
        <h2 className="text-[clamp(14px,2.5vw,25px)] font-dm-sans font-bold text-[#1A3D1C] tracking-wide truncate w-full text-center mb-1.5 group-hover:text-brand-green transition-colors">
          {product.name}
        </h2>

        <div className="flex justify-center items-baseline gap-5 font-dm-sans mb-3">
          <p className="text-[clamp(14px,2vw,23px)] font-bold text-[#b38b59]">
        ...
        <div className="flex flex-wrap justify-between items-center w-full mt-auto pt-5 gap-y-2">
          <StarRating
          ...
          <Link
            ...
            className="flex items-center gap-1 text-[clamp(8px,1vw,12px)] font-medium tracking-[0.1em] text-gray-800 hover:text-brand-green transition-all uppercase underline whitespace-nowrap ml-auto"
          >
            <span>View details</span>
            <svg fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24" className="w-[clamp(10px,1.2vw,14px)] h-[clamp(10px,1.2vw,14px)]"><path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3" /></svg>
          </Link>
        </div>
      </div>
