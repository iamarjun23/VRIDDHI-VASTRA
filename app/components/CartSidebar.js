...
export default function CartSidebar() {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className="fixed right-0 top-0 h-full w-full max-w-md bg-white shadow-2xl z-[101] flex flex-col transform transition-transform translate-x-0 font-dm-sans"
      >
            {/* Header */}
            <div className="p-6 border-b border-gray-100 flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-dm-serif font-medium tracking-wide text-gray-900">Your Cart</h2>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              {cartItems.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center">
                  ...
                  <button 
                    onClick={() => setIsCartOpen(false)}
                    className="mt-6 px-8 py-3 bg-black text-white rounded-full font-dm-sans font-bold text-xs tracking-widest uppercase hover:bg-brand-green transition-all"
                  >
                    Continue Shopping
                  </button>
                </div>
                    ...
                    <div className="flex-1 flex flex-col justify-between py-1">
                      <div>
                        <div className="flex justify-between items-start">
                          <h4 className="font-dm-serif text-[19px] text-gray-900 group-hover:text-brand-green transition-colors leading-snug">{item.name}</h4>
            ...
            {cartItems.length > 0 && (
              <div className="p-6 border-t border-gray-100 space-y-4 bg-gray-50/50">
                <div className="flex justify-between items-center text-lg">
                  <span className="font-dm-sans font-medium text-gray-600">Subtotal</span>
                  <span className="font-dm-sans text-[23px] font-semibold text-gray-900">₹{cartTotal.toLocaleString()}</span>
                </div>
                <p className="text-xs text-gray-500 text-center font-dm-sans">Shipping & taxes calculated at checkout</p>
                <div className="grid grid-cols-1 gap-3 pt-2 font-dm-sans">
                  <button 
                    onClick={handleWhatsAppCheckout}
                    className="w-full flex items-center justify-center gap-3 bg-brand-green text-white py-4 rounded-full font-dm-sans font-bold text-[15px] tracking-[0.1em] uppercase hover:bg-brand-gold transition-all duration-300 shadow-md shadow-brand-green/10 hover:shadow-lg hover:shadow-brand-gold/20 active:scale-[0.98]"
                  >
