...
export default function Navbar({ theme = "light", logo = "", bgColor = "" }) {
  ...
  return (
    ...
    >
      <div className="max-w-[2000px] mx-auto w-full flex justify-between items-center px-4 sm:px-6 md:px-8 lg:px-[30px]">

        {/* Logo Section */}
        <div className="flex items-center shrink-0">
          <Link href="/" className="flex items-center gap-[clamp(0.4rem,1.2vw,1.8rem)] group">
            {logo ? (
              <div className="relative rounded-full overflow-hidden border border-brand-gold/30 shadow-lg group-hover:scale-105 transition-all duration-500 w-[clamp(52px,6.5vw,100px)] h-[clamp(52px,6.5vw,100px)]">
                <Image src={logo} alt="Logo" fill sizes="(max-width: 640px) 52px, (max-width: 1024px) 70px, 100px" className="object-cover" />
              </div>
            ) : (
              <div className="rounded-2xl border border-brand-gold/40 bg-brand-green flex flex-col items-center justify-center shadow-xl relative shrink-0 overflow-hidden transform group-hover:rotate-3 transition-all duration-500 w-[clamp(52px,6.5vw,100px)] h-[clamp(52px,6.5vw,100px)]">
                <div className="absolute inset-0 bg-gradient-to-tr from-black/20 to-transparent"></div>
                <span className="text-[clamp(4px,0.55vw,10px)] font-display leading-none text-center text-brand-gold tracking-[0.3em] uppercase relative z-10 mb-0.5">
                  VRIDDHI
                </span>
                <div className="w-[40%] h-[1px] bg-brand-gold/30 relative z-10 mb-0.5"></div>
                <span className="text-[clamp(4px,0.55vw,10px)] font-display leading-none text-center text-brand-gold tracking-[0.3em] uppercase relative z-10">
                  VASTRA
                </span>
              </div>
            )}
            <h1 className={`
              font-['DM_Serif_Display'] tracking-[0.15em] md:tracking-[0.25em] lg:tracking-[0.35em] transition-all duration-500 whitespace-nowrap
              text-[clamp(14px,2.2vw,50px)]
              ${isLight ? 'text-brand-green' : 'text-white'}`
            }>
              VRIDDHI VASTRA
            </h1>
        ...
        <div className={`hidden lg:flex items-center gap-[clamp(0.75rem,2vw,3rem)] tracking-[0.1em] transition-all duration-500 text-[clamp(11px,1.1vw,19px)] uppercase font-medium 
        ...
        <div className="flex items-center gap-3 md:gap-5 flex-shrink-0 relative">

          {/* Premium Search Pill - Desktop */}
          <div
            className={`hidden md:flex items-center rounded-full transition-all duration-500 ease-in-out overflow-hidden h-[clamp(34px,2.8vw,48px)] backdrop-blur-md cursor-pointer
              ${isSearchOpen
                ? `px-5 w-[clamp(150px,15vw,280px)] ${isLight ? 'border border-brand-green/30 bg-white/40' : 'border border-white/40 bg-white/20'}`
                : `w-[clamp(34px,2.8vw,48px)] justify-center ${isLight ? 'hover:bg-brand-green/5' : 'hover:bg-white/10'}`
              }`}
            onClick={() => {
      ...
      <div 
        className={`absolute top-full left-0 w-full bg-white/95 backdrop-blur-xl border-t border-brand-green/10 shadow-xl transition-all duration-500 ease-in-out md:hidden overflow-hidden ${isSearchOpen ? 'max-h-[80px] opacity-100 py-4' : 'max-h-0 opacity-0 py-0'}`}
      >
        <div className="px-6 relative">
          ...
          />
          <button 
            className="absolute right-10 top-1/2 -translate-y-1/2 text-brand-green/50 hover:text-brand-gold transition-colors"
            onClick={(e) => {
              const input = e.currentTarget.parentElement.querySelector('input');
              if (input && input.value) {
                window.location.href = `/tags?search=${encodeURIComponent(input.value)}`;
              }
            }}
          >
